...
async def upload_csv(file: UploadFile = File(...)):
    try:
        # Save uploaded file as the main dataset
        ...
        if "synthetic_timestamp" not in df.columns:
            from utils import add_synthetic_timestamps
        ...
        num_rows = df.shape[0]
        num_cols = df.shape[1]
        date_min = str(df["synthetic_timestamp"].min())
        date_max = str(df["synthetic_timestamp"].max())
        pass_rate = float((df["Response"] == 1).sum()) / num_rows * 100 if num_rows > 0 else 0.0
